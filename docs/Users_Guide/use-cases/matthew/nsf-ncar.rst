...
.. dropdown:: Instructions
  ...
  .. dropdown:: Log into Derecho
    ...
    passing the ``-Y`` argument to enable X window forwarding be able to view
    images::
  ...
  .. dropdown:: Examine the Plots
    ...
    The images generated by this use case can be viewed using the ``display``
    command if X-window forwarding has been enabled.

    MET line plot::

        display ${WORKING_DIR}/metplus_out/met_plot/line_T2_RMSE_BCMSE.png

    A number of plots of the WRF output can also be found under
    ${WORKING_DIR}/metplus_out/wrf/20161006_00/plots. For example::
